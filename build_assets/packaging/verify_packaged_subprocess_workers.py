...
def _verify_build_modules(build_dir: Path) -> tuple[bool, list[str]]:
    required = [
        ...
        "module.services.mcp.mcp_openai_server.c",
        "module.task_workflow.process_worker.c",
        "module.services.ai.message_builder.c",
        "module.services.ai.provider_config.c",
        "module.services.ai.response_utils.c",
...
def _run_mcp_smoke(exe_path: Path, timeout_sec: float) -> tuple[bool, str, bool]:
    proc = None
    try:
        cmd = [str(exe_path), "--mcp-server"]
        creation_flags = subprocess.CREATE_NO_WINDOW if sys.platform.startswith("win") else 0
        try:
            proc = subprocess.Popen(
                cmd,
        ...
        input_data = "\n".join([
            json.dumps({
                ...
                "params": {
                    ...
                    "clientInfo": {"name": "packaged-smoke", "version": "1.0"},
                },
            }),
            json.dumps({"jsonrpc": "2.0", "id": 2, "method": "tools/list"}),
            "",
        ])
        stdout_data, stderr_data = proc.communicate(input=input_data, timeout=timeout_sec)

        initialize_ok = False
        tools = []
        for raw_line in (stdout_data or "").splitlines():
            line = raw_line.strip()
            if not line:
                continue
            try:
            ...
            if msg.get("id") == 2 and isinstance(msg.get("result"), dict):
                tools = msg.get("result", {}).get("tools") or []

        if not initialize_ok:
            detail = (stderr_data or "").strip()
            return False, f"mcp_initialize_failed:{detail or 'no_initialize_response'}", False

        tool_names = {str(item.get("name") or "") for item in tools if isinstance(item, dict)}
        required = {"openai_raw", "openai_find", "click_client"}
        missing = sorted(name for name in required if name not in tool_names)
        if missing:
            return False, f"mcp_missing_tools:{missing}", False
        return True, "", False
        ...
        if proc is not None:
            try:
                ...
                pass
        return False, "mcp_timeout", False
    except Exception as exc:
        return False, f"{type(exc).__name__}:{exc}", False
    finally:
        if proc is not None and proc.poll() is None:
...
def main() -> int:
    ...
    ok, reason, uac_blocked = _run_mcp_smoke(exe_path, timeout_sec=float(args.timeout))
    if uac_blocked:
        had_uac_block = True
        print("WARN: mcp: uac_elevation_required")
    elif ok:
        print("OK: mcp: smoke_passed")
    else:
        failures.append(("mcp", reason))
        print(f"ERROR: mcp: {reason}")
    ...
    print("OK: ocr_match_workflow_mcp_subprocess_workers_smoke_passed")
    return 0
