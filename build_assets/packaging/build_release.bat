...
if exist "%PROJECT_ROOT%\build_assets\comtypes_gen" rd /s /q "%PROJECT_ROOT%\build_assets\comtypes_gen"
...
for /d %%D in ("%PROJECT_ROOT%\build_assets\comtypes_gen_tmp_*") do (
    if exist "%%~fD" rd /s /q "%%~fD"
)

set "STEP=PREGENERATE_COMTYPES_GEN"
echo [0.7/6] Pre-generate required comtypes typelibs...
set "COMTYPES_GEN_OUT=%PROJECT_ROOT%\build_assets\comtypes_gen"
venv\Scripts\python.exe tools\prepare_ola_comtypes_gen.py --output "%COMTYPES_GEN_OUT%" --dll "%PROJECT_ROOT%\OLA\OLAPlug_x64.dll"
if errorlevel 1 (
    set "ERRMSG=Pre-generate required comtypes typelibs failed"
    goto fail
)
...
if not exist "build_assets\comtypes_gen\*.py" (
    set "ERRMSG=Missing pre-generated comtypes source files in build_assets\\comtypes_gen"
    goto fail
)
venv\Scripts\python.exe build_assets\packaging\stage_packaged_comtypes_gen.py --source-dir "build_assets\comtypes_gen" --dist "%DIST%"
if errorlevel 1 (
    set "ERRMSG=Compile packaged comtypes bytecode failed"
    goto fail
)
for %%f in (__init__.pyc OLAPlugLib.pyc UIAutomationClient.pyc _00020430_0000_0000_C000_000000000046_0_2_0.pyc) do (
    if not exist "%DIST%\comtypes\gen\%%f" (
        set "ERRMSG=Missing packaged comtypes bytecode file in dist: %%f"
        goto fail
    )
)
...
if not exist "%DIST%\plugins\config.json" (
    set "ERRMSG=Missing plugins\config.json"
    goto fail
)
if not exist "%DIST%\OLA\OLAPlug_x64.dll" (
    set "ERRMSG=Missing OLAPlug_x64.dll"
    goto fail
)
for %%f in (__init__.pyc OLAPlugLib.pyc UIAutomationClient.pyc _00020430_0000_0000_C000_000000000046_0_2_0.pyc) do (
    if not exist "%DIST%\comtypes\gen\%%f" (
