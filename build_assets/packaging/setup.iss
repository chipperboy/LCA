...
begin
  ...
  PreserveDir := AddBackslash(BackupRoot) + 'OLA\dict_pics';
  if not BackupDirectoryTree(AddBackslash(Path) + 'OLA\dict_pics', PreserveDir) then
    Exit;

  if not BackupFileIfExists(Path, 'OLA\ola_dict.db', BackupRoot) then
    Exit;
  if not BackupFileIfExists(Path, 'OLA\dict_list.json', BackupRoot) then
    Exit;
  if not BackupFileIfExists(Path, 'OLA\dict_colors.json', BackupRoot) then
    Exit;
  ...
  if DirExists(AddBackslash(Path) + 'OLA') then
  begin
    DeleteFile(AddBackslash(Path) + 'OLA\OLAPlug_x64.dll');
    DeleteFile(AddBackslash(Path) + 'OLA\OLAPlug-beta67_x64.dll');
    DeleteFile(AddBackslash(Path) + 'OLA\OLAPlug_x86.dll');
    DeleteFile(AddBackslash(Path) + 'OLA\OLAPlugServer.py');
    DeleteFile(AddBackslash(Path) + 'OLA\OLAPlugDLLHelper.py');
    DeleteFile(AddBackslash(Path) + 'OLA\OLAPlugCOMLoader.py');
    DeleteFile(AddBackslash(Path) + 'OLA\OLA欧拉插件文档.md');
    DeleteFile(AddBackslash(Path) + 'OLA\欧拉插件文档_beta64.chm');
    DeleteFile(AddBackslash(Path) + 'OLA\欧拉插件文档_beta65.chm');
    DeleteFile(AddBackslash(Path) + 'OLA\__init__.py');
    if DirExists(AddBackslash(Path) + 'OLA\__pycache__') then
      DelTree(AddBackslash(Path) + 'OLA\__pycache__', True, True, True);
  end;
  ...
  if not RestoreDirectoryTree(AddBackslash(BackupRoot) + 'pic_cache', AddBackslash(Path) + 'pic_cache') then
    Exit;
  if not RestoreDirectoryTree(AddBackslash(BackupRoot) + 'OLA\dict_pics', AddBackslash(Path) + 'OLA\dict_pics') then
    Exit;

  if not BackupFileIfExists(BackupRoot, 'OLA\ola_dict.db', Path) then
    Exit;
  if not BackupFileIfExists(BackupRoot, 'OLA\dict_list.json', Path) then
    Exit;
  if not BackupFileIfExists(BackupRoot, 'OLA\dict_colors.json', Path) then
    Exit;
...
begin
  ...
  // 强制删除plugins目录（避免旧配置残留导致兼容性问题）
  DelTree(ExpandConstant('{app}\plugins'), True, True, True);

  if MsgBox('是否保留用户数据？' + #13#10#13#10 +
            '选择"是"将保留配置文件、工作流文件、字库图片、模板图片和备份' + #13#10 +
            '选择"否"将完全删除所有数据', mbConfirmation, MB_YESNO) = IDNO then
  begin
    // 完全删除模式：删除所有用户数据
    DelTree(ExpandConstant('{app}\config.json'), False, True, False);
    // 删除字库相关数据
    DelTree(ExpandConstant('{app}\OLA\dict_pics'), True, True, True);
    DelTree(ExpandConstant('{app}\OLA\ola_dict.db'), False, True, False);
    DelTree(ExpandConstant('{app}\OLA\dict_list.json'), False, True, False);
    DelTree(ExpandConstant('{app}\OLA\dict_colors.json'), False, True, False);
    // 删除备份目录
  ...
  // 保留模式：不删除任何用户数据（config.json、工作流、字库、模板、备份全部保留）
end;
