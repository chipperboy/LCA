...
class WorkflowTaskManagerYoloPrecheckTests(unittest.TestCase):
    ...
    def test_allows_yolo_task_with_plugin_mode_without_native_engine_check(self):
        manager = WorkflowTaskManager({}, "", {"execution_mode": "plugin", "screenshot_engine": "wgc"})
        task = _TaskStub("测试YOLO", "plugin", ["YOLO目标检测"])
