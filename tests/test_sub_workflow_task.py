    ...
    def test_sub_workflow_binds_current_context_for_child_task_modules(self):
        ...
        class FakeWorkflowExecutor:
            ...
            def run(self):
                ...
                context.set_global_var("子变量", "ok", card_id=7, persist=False)

        try:
            set_current_workflow_context(parent_context)
            cards = [
                {"id": 6, "task_type": "线程起点", "parameters": {}},
                {"id": 7, "task_type": "变量提取", "parameters": {}},
            ]
            ...
            self.assertIs(captured_kwargs[0]["workflow_var_context"], seen_contexts[0])
            ...
            self.assertEqual(parent_context.get_global_var("子工作流99.子变量"), "ok")
            self.assertIs(get_workflow_context(), parent_context)
        finally:
