...
class WindowLookupEntrypointsTests(unittest.TestCase):
    ...
    def test_global_settings_plugin_lookup_reuses_shared_exact_match_resolver(self):
        host = _DummyGlobalSettingsLookup()
        ...
        class _PluginManager:
            ...
            def get_preferred_plugin(self, capability):
                _ = capability
                return _Plugin()

        with mock.patch.object(host, '_use_plugin_window_backend', return_value=True):
            with mock.patch(
                'app_core.plugin_bridge.get_plugin_manager',
                return_value=_PluginManager(),
            ):
                with mock.patch.object(
                    _global_settings_lookup_module,
                    'resolve_exact_window_match',
                    ...
                    self.assertEqual(host._find_window_handle('测试窗口'), 202)
